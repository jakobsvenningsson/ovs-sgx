...
unsigned long long ts[9] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
...
static void oftable_enable_eviction(struct oftable *,
                                    ...
                                    size_t n_fields);
#else
static void oftable_enable_eviction(int bridge_id, int table_id,
                                    const struct mf_subfield *fields,
                                    size_t n_fields);
static uint32_t
...
{
    //printf("INIT OFPROTO WITH TYPE %s\n", datapath_type);
...
#else
    sgx_oftable_check_hidden(ofproto->bridge_id);
#endif
 ...
 * constructor function.  See the large comment on 'construct' in structc
                        ...
                        const struct ofproto_table_settings *s)
{
    ...
    HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_oftable_set_name), VAR(ofproto->bridge_id, ui8), VAR(table_id, ui8), PTR(s->name));
    HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_oftable_is_readonly), VAR(ofproto->bridge_id, ui8), VAR(table_id, ui8), VAR(is_read_only, 'd'));
    IF(((struct if_config) { .predicate_fmt = "d" }), VAR(is_read_only, 'd'));
    THEN
        RETURN;
    END
    ...
    BUNDLE_IF_ELSE();
        ...
        HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_oftable_disable_eviction), VAR(ofproto->bridge_id, ui8), VAR(table_id, ui8));
...
#else
    SGX_ofproto_destroy(ofproto->bridge_id);
#endif
    free(ofproto->tables);
...
#else
    return !(SGX_rule_get_flags(rule->ofproto->bridge_id, rule->table_id) & OFTABLE_READONLY);
#endif
}
static enum ofperr
...
#else
    	if(SGX_rule_get_flags(ofproto->bridge_id, i)& OFTABLE_HIDDEN){
#endif
    		n_tables = i;
            break;
        }
...
#else
        ots[i].active_count = htonl(SGX_cls_count(p->bridge_id, i));
#endif
    }
...
#else
        if (SGX_rule_get_flags(p->bridge_id, i) & OFTABLE_HIDDEN) {
            n_tables = i;
            break;
        }
        char buf[100]="";
        SGX_table_name(p->bridge_id, i,buf,100);
        if (strlen(buf)) {
            ovs_strzcpy(ots[i].name, buf, sizeof ots[i].name);
        }
        unsigned int mflows = SGX_table_mflows(p->bridge_id, i);
        if (mflows < ntohl(ots[i].max_entries)) {
            ots[i].max_entries = htonl(mflows);
        }
                    ...
                    uint16_t out_port, struct list *rules)
{
    ...
    FOR_EACH_MATCHING_TABLE (table, table_id, ofproto) {
        ...
        CLS_CURSOR_FOR_EACH (rule, cr, &cursor) {
            if (rule->pending) {
                VLOG_INFO("Error pending normal\n");
...
exit:
    ...
    /*HCALL_SIMPLE(
        CONFIG(.function_id = hotcall_ecall_collect_rules_loose_c),
        VAR(ofproto->bridge_id, ui8),
    ...
    HCALL_SIMPLE(
        CONFIG(.function_id = hotcall_ecall_collect_rules_loose_r),
        VAR(ofproto->bridge_id, ui8),
        VAR(ofproto->n_tables, 'd'),
        PTR(buf),
        VAR(n, 'u'),
    ...
    int buf_size= 32;//SGX_collect_rules_loose_c(ofproto->bridge_id, ofproto->n_tables,table_id,match);
    //printf("%d\n", buf_size);
    if(buf_size){
    	int lp;
    	struct cls_rule *buf[buf_size];
        int n = SGX_collect_rules_loose_r(ofproto->bridge_id, ofproto->n_tables, buf,buf_size,table_id,match);
    	for(lp=0;lp<n;lp++){
    		struct rule *rule=rule_from_cls_rule(buf[lp]);
...
exit:
    ...
    struct cls_rule *buf[default_buffer_size], **cr_buf = buf;

    BUNDLE_BEGIN();

    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_collect_rules_strict_c),
        VAR(ofproto->bridge_id, 'u'), VAR(ofproto->n_tables, 'd'),
        VAR(table_id, 'u'), PTR(match), VAR(priority, 'u'), VAR(n, 'u')
    );

    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_collect_rules_strict_r),
        VAR(ofproto->bridge_id, 'u'), VAR(ofproto->n_tables, 'd'), PTR(buf),
        VAR(default_buffer_size, 'u'), VAR(table_id, 'u'), PTR(match),
        VAR(priority, 'u'), VAR(m, 'u')
    );
    ...
    if(overflow > 0) {
        HCALL_SIMPLE(
            CONFIG(.function_id = hotcall_ecall_collect_rules_strict_r),
            VAR(ofproto->bridge_id, 'u'), VAR(ofproto->n_tables, 'd'),
            PTR(buf_extended), VAR(n, 'u'), VAR(table_id, 'u'), PTR(match),
            VAR(priority, 'u'), VAR(m, 'u')
        );
        cr_buf = buf_extended;
    }
...
flow_stats_ds(struct rule *rule, struct ds *results)
{
...
#else
    ds_put_format(results, "priority=%u, ", SGX_cr_priority(rule->ofproto->bridge_id, &rule->cr));
#endif
    ...
    unsigned int priority_sgx= SGX_cls_rule_format(rule->ofproto->bridge_id,  &rule->cr,&megamatch);
...
#else
        HMAP_FOR_EACH_WITH_HASH (op, hmap_node,
        		SGX_cls_rule_hash(ofproto->bridge_id, cls_rule, table_id),
                                         &ofproto->deletions) {
            if (SGX_cls_rule_equal(ofproto->bridge_id, cls_rule, &op->rule->cr)) {
                return true;
            }
        }
    }
         ...
         const struct ofputil_flow_mod *fm, const struct ofp_header *request)
{
    ...
    victim = rule_from_cls_rule(cls_rule_victim);
    if (victim) {
    ...
    if (fm->flags & OFPFF_CHECK_OVERLAP
            && SGX_cr_rule_overlaps(ofproto->bridge_id, table_id,&rule->cr)) {
        SGX_cls_rule_destroy(ofproto->bridge_id, &rule->cr);
        ofproto->ofproto_class->rule_dealloc(rule);
        return OFPERR_OFPFMFC_OVERLAP;
    }
               ...
               const struct ofp_header *request, struct list *rules)
{
    ...
    #ifdef BUNDLE
    unsigned int len = list_size(rules);
    struct rule *rls[len];
    int rls_prio[len];
    bool rls_is_hidden[len];
    size_t j = 0;
    LIST_FOR_EACH (rule, ofproto_node, rules) { rls[j++] = rule; }

    if(len > 1) {
        BUNDLE_BEGIN();
        MAP(((struct map_config) { .function_id = hotcall_ecall_cr_priority, .n_iters = &len }),
            VAR(ofproto->bridge_id, 'u'),
            VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
            VECTOR(rls_prio, 'u')
        );
        BUNDLE_END();
        for(int i = 0; i < len; ++i) rls_is_hidden[i] = rls_prio[i] > UINT16_MAX;
        ofopgroup_submit(group, NULL, NULL, rls_is_hidden);

    } else {
        //HCALL_SIMPLE(CONFIG(.function_id = hotcall_ecall_cr_priority), VAR(ofproto->bridge_id, 'u'), PTR(&rls[0]->cr), VAR(rls_prio, 'u'));
        //rls_is_hidden[0] = rls_prio[0] > UINT16_MAX;
        ofopgroup_submit(group, NULL, NULL, NULL);
    }
    #else
    ofopgroup_submit(group, NULL, NULL, NULL);
...
#ifdef OPTIMIZED
static enum ofperr
modify_flows_loose_optimized(struct ofproto *ofproto, struct ofconn *ofconn,
                               const struct ofputil_flow_mod *fm,
                               const struct ofp_header *request)
{
    ...
    size_t buffer_size = 128, total_rules = 0;

    struct cls_rule **cr_rules;
    struct cls_rule *cr_rules_[buffer_size];
    cr_rules = cr_rules_;

    bool *rule_is_hidden;
    bool rule_is_hidden_[buffer_size];
    rule_is_hidden = rule_is_hidden_;

    bool *rule_is_mod;
    bool rule_is_mod_[buffer_size];
    rule_is_mod = rule_is_mod_;

    bool postpone = false;

    size_t n = SGX_modify_flows_loose(ofproto->bridge_id,
                                         fm->table_id,
                                         ofproto->n_tables,
                                         0,
                                         &fm->match,
    ...
    if(postpone) {
        ...
        size_t leftovers = total_rules - n;
        struct cls_rule *extended_cr_rules[total_rules];
        struct rule_is_mod *extended_rule_is_mod[total_rules];
        struct rule_is_hidden *extended_rule_is_hidden[total_rules];
        if(leftovers > 0) {
            memcpy(extended_cr_rules, cr_rules_, sizeof(cr_rules_));
            memcpy(extended_rule_is_mod, rule_is_mod_, sizeof(rule_is_mod_));
            memcpy(extended_rule_is_hidden, rule_is_hidden_, sizeof(rule_is_hidden_));

            size_t dummy;
            SGX_modify_flows_loose(ofproto->bridge_id, fm->table_id,
                                                 ofproto->n_tables,
                                                 n,
                                                 &fm->match,
                                                 extended_cr_rules + n,
                                                 ...
                                                 fm->cookie_mask,
                                                 fm->out_port,
                                                 extended_rule_is_mod + n,
                                                 extended_rule_is_hidden + n,
                                                 &postpone,
                                                 &dummy);
            ...
            cr_rules = extended_cr_rules;
            rule_is_mod = extended_rule_is_mod;
            rule_is_hidden = extended_rule_is_hidden;
        }
...
{
    #ifdef OPTIMIZED
    return modify_flows_loose_optimized(ofproto, ofconn, fm, request);
    #else
    ...
    if (error) {
        ...
        return modify_flows_add(ofproto, ofconn, fm, request);
         ...
         enum ofperr err;
         err = modify_flows__(ofproto, ofconn, fm, request, &rules);
        return err;
    }
    #endif
}
...
static enum ofperr
modify_flow_strict_optimized(struct ofproto *ofproto, struct ofconn *ofconn,
                   const struct ofputil_flow_mod *fm,
                   const struct ofp_header *request)
{
    size_t buffer_size = fm->table_id == 0xff ? ofproto->n_tables : 1;
    struct cls_rule *cls_rule_buffer[buffer_size];
    bool rule_is_mod[buffer_size];
    bool rule_is_hidden[buffer_size];
    bool ofproto_postpone = false;

    size_t n = SGX_modify_flows_strict(ofproto->bridge_id,
                                        fm->table_id,
                                        ofproto->n_tables,
                                        &fm->match,
                                        fm->priority,
                   ...
                   const struct ofp_header *request)
{
    ...
    int error;
    #ifdef OPTIMIZED
    return modify_flow_strict_optimized(ofproto, ofconn, fm, request);
    #else
...
#ifdef OPTIMIZED
static void
ofproto_rule_send_removed_del(struct rule *rule, uint8_t reason, bool rule_is_hidden, unsigned int priority, struct match match)
{
               ...
               const struct ofp_header *request, struct list *rules)
{
    ...
    unsigned int table_ids[len], hashes[len];

    LIST_FOR_EACH_SAFE (rule, next, ofproto_node, rules) {
        //table_ids[i] = rule->table_id;
        //crs[i] = &rule->cr;
        rls[i++] = rule;
    }
    ...
    MAP(
        ...
        VAR(ofproto->bridge_id, 'u'),
        //VECTOR(crs, 'p', &len, .dereference = true),
        VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
        VECTOR(rls_prio, 'u')
    );

    MAP(
        ((struct map_config) { .function_id = hotcall_ecall_cls_rule_hash, .n_iters = &len }),
        VAR(ofproto->bridge_id, 'u'),
        //VECTOR(crs, 'p', &len, .dereference = true),
        VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
    ...
    FOR_EACH(
        ((struct for_each_config) { .function_id = hotcall_ecall_cls_remove, .n_iters = &len }),
        VAR(ofproto->bridge_id, 'u'),
        //VECTOR(table_ids, 'u'),
        //VECTOR(crs, 'p', &len, .dereference = true)
        VECTOR(rls, 'u', &len, .dereference = true, .member_offset = offsetof(struct rule, table_id)),
    ...
    FOR_EACH(
        ((struct for_each_config) { .function_id = hotcall_ecall_evg_remove_rule, .n_iters = &len }),
        VAR(ofproto->bridge_id, 'u'),
        //VECTOR(table_ids, 'u'),
        //VECTOR(crs, 'p', &len, .dereference = true)
        VECTOR(rls, 'u', &len, .dereference = true, .member_offset = offsetof(struct rule, table_id)),
    ...
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_oftable_is_other_table),
        VAR(ofproto->bridge_id, 'u'),
        VECTOR(rls, 'u', &len, .dereference = true, .member_offset = offsetof(struct rule, table_id)),
    ...
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_oftable_update_taggable),
        VAR(ofproto->bridge_id, 'u'),
        VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
    ...
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_cr_priority),
        VAR(ofproto->bridge_id, 'u'),
        VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
        VECTOR(rls_prio, 'u')
    );
    ...
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_cls_rule_hash),
        VAR(ofproto->bridge_id, 'u'),
        VECTOR(rls, 'p', &len, .dereference = true, .member_offset = offsetof(struct rule, cr)),
        VECTOR(rls, 'u', &len, .dereference = true, .member_offset = offsetof(struct rule, table_id)),
        VECTOR(rls_hashes, 'u')
    ...
    for(int i = 0; i < len; ++i) {
        ...
        ofproto->ofproto_class->rule_destruct(rls[i]);
    }
    #else
    LIST_FOR_EACH_SAFE (rule, next, ofproto_node, rules) {
...
delete_flows_loose_optimized(struct ofproto *ofproto, const struct ofputil_flow_mod *fm, struct ofconn *ofconn, const struct ofp_header *request) {
    ...
    struct cls_rule *cls_rule_buffer_[buffer_size];
    struct cls_rule **cls_rule_buffer = cls_rule_buffer_;

    bool rule_is_mod_[buffer_size];
    bool *rule_is_mod = rule_is_mod_;

    bool rule_is_hidden_[buffer_size];
    bool *rule_is_hidden = rule_is_hidden_;

    uint32_t rule_hashes_[buffer_size];
    uint32_t *rule_hashes = rule_hashes_;

    uint32_t rule_priorities_[buffer_size];
    uint32_t *rule_priorities = rule_priorities_;

    struct match matches_[buffer_size];
    struct match *matches = matches_;

    bool ofproto_postpone = false;

    size_t n = SGX_delete_flows_loose(ofproto->bridge_id,
                                        fm->table_id,
                                        ofproto->n_tables,
                                        0,
                                        &fm->match,
    ...
    bool extended_rule_is_mod[total_rules];
    bool extended_rule_is_hidden[total_rules];
    uint32_t extended_rule_hashes[total_rules];
    uint32_t extended_rule_priorities[total_rules];
    struct match extended_matches[total_rules];

    if(leftovers > 0) {
        memcpy(extended_cls_rule_buffer, cls_rule_buffer_, sizeof(cls_rule_buffer_));
        memcpy(extended_rule_is_mod, rule_is_mod_, sizeof(rule_is_mod_));
        memcpy(extended_rule_is_hidden, rule_is_hidden_, sizeof(rule_is_hidden_));
        memcpy(extended_rule_hashes, rule_hashes_, sizeof(rule_hashes_));
        memcpy(extended_rule_priorities, rule_priorities_, sizeof(rule_priorities_));
        memcpy(extended_matches, matches_, sizeof(matches_));
        size_t dummy;
        size_t m = SGX_delete_flows_loose(ofproto->bridge_id,
                                            fm->table_id,
                                            ofproto->n_tables,
                                            0,
                                            &fm->match,
                                            ...
                                            extended_rule_hashes + n,
                                            extended_rule_priorities + n,
                                            extended_matches + n,
                                            leftovers,
                                            extended_rule_is_mod + n,
                                            extended_rule_is_hidden + n,
        ...
        cls_rule_buffer = extended_cls_rule_buffer;
        rule_is_mod = extended_rule_is_mod;
        rule_is_hidden = extended_rule_is_hidden;
        rule_hashes = extended_rule_hashes;
        rule_priorities = extended_rule_priorities;
                   ...
                   const struct ofp_header *request)
{
    ...
    #ifdef OPTIMIZED
    return delete_flows_loose_optimized(ofproto, fm, ofconn, request);
    #else
    error = collect_rules_loose(ofproto, fm->table_id, &fm->match,
                                fm->cookie, fm->cookie_mask,
                                fm->out_port, &rules);
...
#ifdef OPTIMIZED
static enum ofperr
delete_flow_strict_optimized(struct ofproto *ofproto, struct ofconn *ofconn,
                   const struct ofputil_flow_mod *fm,
                   const struct ofp_header *request)
{
    enum ofperr error;
    ...
    size_t n = SGX_delete_flows_strict(ofproto->bridge_id,
                                        fm->table_id,
                                        ofproto->n_tables,
                                        &fm->match,
                                        fm->priority,
                   ...
                   const struct ofp_header *request)
{
    ...
    return delete_flow_strict_optimized(ofproto, ofconn, fm , request);
...
#else
    // rule_is_hidden contains SGX
    if ((is_hidden ? *is_hidden : ofproto_rule_is_hidden(rule)) || !rule->send_flow_removed) {
        return;
    }
    SGX_minimatch_expand(rule->ofproto->bridge_id, &rule->cr, &fr.match);
    fr.priority = priority ? *priority : SGX_cr_priority(rule->ofproto->bridge_id, &rule->cr);
#endif
    fr.cookie = rule->flow_cookie;
    fr.reason = reason;
    fr.table_id = rule->table_id;
...
ofproto_rule_update_used(struct rule *rule, long long int used)
{
    if (used > rule->used) {
        #ifdef SGX
        rule->used = used;
        SGX_rule_update_used(rule->ofproto->bridge_id, &rule->cr, rule_eviction_priority(rule));
        #else
                                    ...
                                    struct list *msgs)
{
...
#elif OPTIMIZED
    unsigned int priority;
    SGX_minimatch_expand_and_get_priority(rule->ofproto->bridge_id, &rule->cr, &match, &priority);
    fu.match = &match;
    ...
    HCALL_BUNDLE(
        ...
        VAR(rule->ofproto->bridge_id, ui8), PTR(&rule->cr, 'p'), PTR(&match, 'p')
    );
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_cr_priority),
        VAR(rule->ofproto->bridge_id,'u'), PTR(&rule->cr, 'p'), VAR(fu.priority, 'u')
    );
    BUNDLE_END();
    fu.match = &match;
#else
    SGX_minimatch_expand(rule->ofproto->bridge_id, &rule->cr,&match);
    fu.match = &match;
                                        ...
                                        struct list *rules)
{
    ...
    buf_size=SGX_collect_ofmonitor_util_c(ofproto->bridge_id, ofproto->n_tables,m->table_id,&m->match);

    //we need to allocate the buffer
    if(buf_size){
    	struct cls_rule *buf[buf_size];
    	//We need to populate our buffer
    	SGX_collect_ofmonitor_util_r(ofproto->bridge_id, ofproto->n_tables,buf,buf_size,m->table_id,&m->match);
    	int lp;
    	for(lp=0;lp<buf_size;lp++){
...
#else
        if (((m->table_id == 0xff
              ? !(SGX_rule_get_flags(ofproto->bridge_id, rule->table_id) & OFTABLE_HIDDEN)
        	  : m->table_id == rule->table_id))
          && SGX_cls_rule_is_loose_match(ofproto->bridge_id, &rule->cr, &m->match)) {
#endif
        	ofproto_collect_ofmonitor_refresh_rule(m, rule, seqno, rules);
        }
    }
...
handle_openflow__(struct ofconn *ofconn, const struct ofpbuf *msg)
{
    ...
    case OFPTYPE_FLOW_STATS_REQUEST:
        #ifdef BENCHMARK_DUMP_FLOWS
        {
            enum ofperr res;
            BENCHMARK(handle_flow_stats_request, res, ofconn, oh);
            return res;
        }
        #else
...
ofopgroup_complete(struct ofopgroup *group, uint16_t *_vid, uint16_t *_vid_mask, bool *is_hidden)
{
    ...
    LIST_FOR_EACH_SAFE (op, next_op, group_node, &group->ops) {
        ...
        // rule_is_modifiable contains SGX
        if (!(op->error
              || (is_hidden ? is_hidden[i++] : ofproto_rule_is_hidden(rule))
...
#elif OPTIMIZED
                vid_mask = rule->tmp_storage_vid_mask_exist
                    ? rule->tmp_storage_vid_mask
                    : SGX_minimask_get_vid_mask(rule->ofproto->bridge_id, &rule->cr);
                rule->tmp_storage_vid_mask_exist = 0;
#elif BUNDLE123
                BUNDLE_BEGIN();
                HCALL_BUNDLE(
                    CONFIG(.function_id = hotcall_ecall_minimask_get_vid_mask),
                    VAR(rule->ofproto->bridge_id, 'u'), PTR(&rule->cr), VAR(vid_mask, ui16)
                );
                HCALL_BUNDLE(
                    CONFIG(.function_id = hotcall_ecall_miniflow_get_vid),
                    VAR(rule->ofproto->bridge_id, 'u'), PTR(&rule->cr), VAR(vid, ui16)
                );
                BUNDLE_END();
#else
                vid_mask = SGX_minimask_get_vid_mask(rule->ofproto->bridge_id, &rule->cr);
#endif
...
#ifndef SGX
                		vid = miniflow_get_vid(&rule->cr.match.flow);
#elif OPTIMIZED
                        vid = rule->tmp_storage_vid_exist
...
ofoperation_create(struct ofopgroup *group, struct rule *rule,
                   ...
                   enum ofp_flow_removed_reason reason, uint32_t *hash)
{
    struct ofproto *ofproto = group->ofproto;
    struct ofoperation *op;
...
#else
        (hash ? *hash : SGX_cls_rule_hash(rule->ofproto->bridge_id, &rule->cr, rule->table_id)));
#endif
    }
...
ofproto_evict(struct ofproto *ofproto)
{
    ...
    BUNDLE_BEGIN();
        ...
        MAP(
            ((struct map_config) {
                .function_id = hotcall_ecall_oftable_cls_count, .n_iters = &n_eviction_tables
            }),
            VAR(ofproto->bridge_id, 'u'),
            VECTOR(eviction_tables, 'd'),
            VECTOR(table_n_flows, 'd')
        );

        MAP(
            ((struct map_config) {
                .function_id = hotcall_ecall_oftable_mflows, .n_iters = &n_eviction_tables
            }),
            VAR(ofproto->bridge_id, 'u'),
            VECTOR(eviction_tables, 'd'),
            VECTOR(table_max_flows, 'd')
        );
    ...
    BEGIN_FOR((struct for_config) {
        .n_iters = &n_eviction_tables
    });
            BEGIN_WHILE(
                ((struct while_config) { .predicate_fmt = "d>d" }),
                VECTOR(table_n_flows, 'd', &n_eviction_tables), VECTOR(table_max_flows, 'd', &n_eviction_tables)
            );
                ...
                IF(((struct if_config) {
                    ...
                    VECTOR(cr_rules_buf, 'p', &total_overflows, .dereference = false, .member_offset = -offsetof(struct rule, cr), .vector_offset = &n),
                    VECTOR(cr_rules_buf, 'p', &total_overflows, .dereference = true, .member_offset = -offsetof(struct rule, cr) + offsetof(struct rule, pending), .vector_offset = &n)
                );
                THEN
                    RETURN;
                END
                ...
                HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_plus_one), VAR(n, 'd'));
                HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_minus_one), VECTOR(table_n_flows, 'd', &n_eviction_tables));
    ...
    for(int j = 0; j < n; ++j) {
    	...
    	for(i=0;i<ofproto->n_tables;i++){
    		while(SGX_cls_count(ofproto->bridge_id, i) > SGX_table_mflows(ofproto->bridge_id, i)
    				&& SGX_eviction_fields_enable(ofproto->bridge_id, i)){
                  SGX_choose_rule_to_evict(ofproto->bridge_id, i, &cls_rule_t);
                  struct rule *rule;
                  rule = rule_from_cls_rule(cls_rule_t);
...
#else
	SGX_evg_remove_rule(rule->ofproto->bridge_id, rule->table_id, &rule->cr);
#endif
}
...
eviction_group_add_rule(struct rule *rule)
{
    ...
    int is_eviction_enabled = false;
    unsigned int evg_grp_prio = eviction_group_priority(0), rule_evg_prio = rule_eviction_priority(rule);

    BUNDLE_BEGIN();

    HCALL_BUNDLE(CONFIG(.function_id = hotcall_ecall_is_eviction_fields_enabled), VAR(ofproto->bridge_id, ui8), VAR(rule->table_id, ui8), VAR(is_eviction_enabled, 'd'));
    IF(
        ((struct if_config) { .predicate_fmt = "d&(d|d)" }), VAR(is_eviction_enabled, 'd'), VAR(rule->hard_timeout, 'd'), VAR(rule->idle_timeout, 'd')
    );
    THEN
        HCALL_BUNDLE(
            CONFIG(.function_id = hotcall_ecall_evg_add_rule),
            VAR(rule->ofproto->bridge_id, ui8), VAR(rule->table_id, ui8), PTR(&rule->cr), VAR(evg_grp_prio, 'u'), VAR(rule_evg_prio, 'u')
        );
    END
...

{
    ...
    int cr_field_offset = offsetof(struct rule, cr);
    unsigned int evg_grp_prio = eviction_group_priority(0), default_buffer_size = 128, random_v = random_uint32();
    long long int boot_time = time_boot_msec(); unsigned int boot_time_u = boot_time >> 10;
    int is_eviction_enabled = 1;
    unsigned int n_cr_rules, n;

    bool no_change = false;

    HCALL_SIMPLE(
        CONFIG(.function_id = hotcall_ecall_oftable_enable_eviction),
        VAR(bridge_id, 'u'), VAR(table_id, 'u'), PTR(fields), VAR(n_fields, 'u'), VAR(random_v, 'u'), VAR(no_change, 'b')
    );
    if(no_change) return;

    BUNDLE_BEGIN();

    struct cls_rule *cr_buf[default_buffer_size];
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_oftable_enable_eviction_r),
        VAR(bridge_id, 'u'), PTR(cr_buf), VAR(default_buffer_size, 'u'), VAR(table_id, 'u'), VAR(n_cr_rules, 'u'), VAR(n, 'u')
    );
    HCALL_BUNDLE(
        CONFIG(.function_id = hotcall_ecall_is_eviction_fields_enabled),
        VAR(bridge_id, ui8),
    ...
    unsigned int n_filtered = 0, rule_prs[default_buffer_size];
    void *cr_buf_out[default_buffer_size];
    for(int i = 0; i < n; ++i) {
        struct rule *rule = rule_from_cls_rule(cr_buf[i]);
        if(is_eviction_enabled && (rule->hard_timeout || rule->idle_timeout)) {
            rule_prs[n_filtered] = rule_eviction_priority(rule);
            cr_buf_out[n_filtered++] = cr_buf[i];
        }
    }
    ...
    unsigned int rule_eviction_priority = 0;
    ...
    BEGIN_FOR((struct for_config) {
        ...
        THEN
            HCALL(
                CONFIG(.function_id = hotcall_ecall_rule_eviction_priority),
                VECTOR(cr_buf, 'p', &n, .dereference = true, .member_offset = -cr_field_offset),
                VAR(boot_time_u, 'u'),
                VAR(rule_eviction_priority, 'u')
            );
            HCALL(
                CONFIG(.function_id = hotcall_ecall_evg_add_rule),
                VAR(bridge_id, 'u'),
    ...
    if(buf_size){
        int lp;
        struct cls_rule *buf[buf_size];
        SGX_oftable_enable_eviction_r(bridge_id, buf,buf_size,table_id, NULL);
    ...
    CLOSE
    #ifdef BENCHMARK_ENABLE_EVICTION
    SHOWTIME
    SHOWTIME5
    #endif
...
ofproto_get_vlan_usage(struct ofproto *ofproto, unsigned long int *vlan_bitmap)
{
...
#elif OPTIMIZED
    size_t default_buffer_size = 32, n_vlan;
    uint16_t vlan_buffer[default_buffer_size];
    size_t n = SGX_ofproto_get_vlan_usage(ofproto->bridge_id, default_buffer_size, vlan_buffer, 0, -1, &n_vlan);
    ...
    if(leftover_vlan > 0) {
        ...
        SGX_ofproto_get_vlan_usage(ofproto->bridge_id, leftover_vlan, extended_buffer + n, n, -1, &n_vlan);
   ...
   //1. first we determine the size of the buffer
    int buf_size;
    buf_size = SGX_ofproto_get_vlan_usage_c(ofproto->bridge_id);
    //2. We allocate the buffer
    if(buf_size){
    	int lp;
    	uint16_t buf[buf_size];
    	SGX_ofproto_get_vlan_usage__r(ofproto->bridge_id, buf,buf_size);
    	for(lp=0;lp<buf_size;lp++){
