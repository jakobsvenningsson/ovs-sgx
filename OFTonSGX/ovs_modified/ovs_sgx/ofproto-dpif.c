...
#else
    //VLOG_ERR("SGX_readonly_set");
    SGX_readonly_set(ofproto->up.bridge_id, TBL_INTERNAL);
#endif
    ofproto->n_hit = 0;
    ofproto->n_missed = 0;
...
wait(struct ofproto *ofproto_)
{
    ...
    if (ofproto->backer->need_revalidate) {
        ...
        poll_immediate_wake();
        VLOG_DBG_RL(&rl, "evalidate in ofproto_wait_cb()");
...
facet_check_consistency(struct facet *facet)
{
    ...
    if (rule != facet->rule) {
        if (!VLOG_DROP_WARN(&rl)) {
            ...
            unsigned int priority_sgx= SGX_cls_rule_format(rule->up.ofproto->bridge_id, &facet->rule->up.cr,&megamatch);
            ...
            unsigned int priority_sgx1= SGX_cls_rule_format(rule->up.ofproto->bridge_id,  &rule->up.cr,&megamatch1);
...
tag_the_flow(struct xlate_ctx *ctx, struct rule_dpif *rule)
{
    ...
    //VLOG_ERR("tag_the_flow");
    int bridge_id = ofproto->up.bridge_id;
    if (SGX_is_sgx_other_table(bridge_id, table_id)) {
                   ...
                   uint16_t in_port, uint8_t table_id, bool may_packet_in)
{
  //VLOG_ERR("xlate_table_action");
    ...
    if (ctx->recurse < MAX_RESUBMIT_RECURSION) {
        ...
        //VLOG_ERR("rule_dpif_lookup__222");
...
{
  //VLOG_ERR("update_learning_table");
...
{
  //VLOG_ERR("ofproto_dpif_self_check__");
