...
*/
struct
sgx_cls_rule* node_search(int bridge_id,
																 const struct cls_rule *out) {
...
struct
sgx_cls_rule* node_insert(int bridge_id,
																 uint32_t hash) {
...
void
ecall_ofproto_init_tables(int bridge_id,
													int n_tables) {
...
void
ecall_cls_rule_init_i(int bridge_id,
...
int
ecall_cr_rule_overlaps(int bridge_id, int table_id,
													 struct cls_rule * out) {
...
void
ecall_classifier_replace(int bridge_id,
															int table_id ,
															struct cls_rule *o_cls_rule,
															struct cls_rule **cls_rule_rtrn) {
	struct sgx_cls_rule * sgx_cls_rule = node_search(bridge_id, o_cls_rule);
	struct cls_rule * cls_rule = classifier_replace(&SGX_oftables[bridge_id][table_id].cls, &sgx_cls_rule->cls_rule);
	if(cls_rule){
...
//10. rule_get_flags
enum
oftable_flags ecall_rule_get_flags(int bridge_id, int table_id) {
...
static uint32_t
eviction_group_hash_rule(int bridge_id,
												 int table_id,
												 struct cls_rule *cls_rule) {
...
size_t
ecall_evg_add_rule(int bridge_id,
									 int table_id,
									 struct cls_rule *o_cls_rule,
									 uint32_t priority,
	...
	struct sgx_cls_rule * sgx_cls_rule= node_search(bridge_id, o_cls_rule);
	struct eviction_group *evg;
	evg = ecall_evg_find(bridge_id, table_id, eviction_group_hash_rule(bridge_id, table_id, &sgx_cls_rule->cls_rule), priority);
...
//16
void
ecall_evg_group_resize(int bridge_id,
														int table_id,
														struct cls_rule *o_cls_rule,
														size_t priority,
														struct eviction_group *evg) {
...
*/
void
ecall_evg_destroy(int bridge_id,
									int table_id,
									struct eviction_group *evg) {
...
int
ecall_evg_remove_rule(int bridge_id,
										 int table_id,
										 struct cls_rule *o_cls_rule) {
	struct sgx_cls_rule * sgx_cls_rule = node_search(bridge_id, o_cls_rule);
...
void
ecall_cls_remove(int bridge_id,
								 int table_id,
								 struct cls_rule *o_cls_rule) {
	struct sgx_cls_rule *sgx_cls_rule = node_search(bridge_id, o_cls_rule);
...
void
ecall_choose_rule_to_evict(int bridge_id,
													 int table_id,
													 struct cls_rule **o_cls_rule) {
	*o_cls_rule = NULL;
...
struct cls_rule*
ecall_choose_rule_to_evict_p(int bridge_id,
														 int table_id,
														 struct cls_rule **o_cls_rule,
														 struct cls_rule *replacer) {
...
void
ecall_table_mflows_set(int bridge_id,
														int table_id,
														unsigned int value){
...
void
ecall_minimatch_expand(int bridge_id,
											struct cls_rule *o_cls_rule,
											struct match *dst) {
	struct sgx_cls_rule * sgx_cls_rule = node_search(bridge_id, o_cls_rule);
...
unsigned int
ecall_cr_priority(int bridge_id,
									const struct cls_rule *o_cls_rule) {
	struct sgx_cls_rule * sgx_cls_rule= node_search(bridge_id, o_cls_rule);
...
void
ecall_cls_find_match_exactly(int bridge_id, int table_id,
                             const struct match *target,
                             unsigned int priority,
														 struct cls_rule ** o_cls_rule) {
	struct cls_rule * cls_rule=classifier_find_match_exactly(&SGX_oftables[bridge_id][table_id].cls,
																													 target,
																													 priority);
	*o_cls_rule = cls_rule
...
next_visible_table(int bridge_id,
...
static struct oftable *
first_matching_table(int bridge_id,
										 int ofproto_n_tables,
										 uint8_t table_id) {
    if (table_id == 0xff) {
...
static struct oftable *
next_matching_table(int bridge_id,
										int ofproto_n_tables,
...
int
ecall_femt_ccfe_c(int bridge_id,
									int ofproto_n_tables,
									uint8_t table_id,
									const struct match *match) {
	struct oftable *table;
...
void
ecall_femt_ccfe_r(int bridge_id,
									int ofproto_n_tables,
									struct cls_rule **buf,
									int elem,
									uint8_t table_id,
...
int
ecall_collect_ofmonitor_util_c(int bridge_id,
															 int ofproto_n_tables,
															 int table_id,
...
void
ecall_collect_ofmonitor_util_r(int bridge_id,
															 int ofproto_n_tables,
															 struct cls_rule **buf,
															 int elem,int table_id,
															 const struct minimatch *match) {
	struct cls_rule target;
	const struct oftable *table;
	int p = 0;
...
//50.2
void
ecall_fet_ccfes_r(int bridge_id,
											 struct cls_rule **buf,
											 int elem) {
	int p = 0;
	int i;
...
//51.2 REQUEST
void
ecall_fet_ccfe_r(int bridge_id,
								 struct cls_rule **buf,
								 int elem) {
	int p = 0;
	int i;
...
int ecall_femt_c(int bridge_id, int ofproto_n_tables,uint8_t table_id,const struct match *match,unsigned int priority){
	struct oftable *table;
	struct cls_rule cr;
...
void ecall_femt_r(int bridge_id, int ofproto_n_tables,struct cls_rule **buf,int elem,uint8_t table_id,const struct match *match,unsigned int priority)
{
	struct oftable *table;
	struct cls_rule cr;
	ecall_cls_rule_init_i(bridge_id, &cr, match ,priority);
...
int ecall_ccfe_c(int bridge_id, int table_id){
	int count=0;
	struct cls_cursor cursor;
	struct sgx_cls_rule * rule;
...
void ecall_ccfe_r(int bridge_id, struct cls_rule **buf,int elem,int table_id){
	struct cls_cursor cursor;
	struct sgx_cls_rule * rule;
	int p=0;
	cls_cursor_init(&cursor,&SGX_oftables[bridge_id][table_id].cls, NULL);
...
void ecall_oftable_enable_eviction(int bridge_id, int table_id, const struct mf_subfield *fields, size_t n_fields,uint32_t random_v, bool *no_change)
{
	if(SGX_oftables[bridge_id][table_id].eviction_fields
...
void ecall_oftable_destroy(int bridge_id, int table_id){
...
void ecall_ofproto_destroy(int bridge_id){
	int i;
	for(i=0;i<SGX_n_tables[bridge_id];i++){
...
unsigned int ecall_total_rules(int bridge_id){
	int i;
...
//38 Table name
void ecall_table_name(int bridge_id, int table_id,char *buf,size_t len){
...
int ecall_cls_rule_is_loose_match(int bridge_id, struct cls_rule *o_cls_rule,const struct minimatch *criteria){
	struct sgx_cls_rule * sgx_cls_rule=node_search(bridge_id,o_cls_rule);
...
//1. Classifier_lookup
void ecall_cls_lookup(int bridge_id, struct cls_rule **o_cls_rule,int table_id,const struct flow *flow,
...
//2. cls_rule priority
unsigned int ecall_cls_rule_priority(int bridge_id, struct cls_rule *o_cls_rule){
...
int ecall_desfet_ccfes_c(int bridge_id){
	int i;
	int count=0;
	for(i=0;i<SGX_n_tables[bridge_id];i++){
		struct sgx_cls_rule *rule,*next_rule;
...
//50.2
void ecall_desfet_ccfes_r(int bridge_id, struct cls_rule **buf,int elem){
	int p=0;
	int i;
	for(i=0;i<SGX_n_tables[bridge_id];i++){
			struct sgx_cls_rule *rule,*next_rule;
...
unsigned int ecall_cls_rule_format(int bridge_id, const struct cls_rule *o_cls_rule,struct match *megamatch){
	 struct sgx_cls_rule * sgx_cls_rule;
	 sgx_cls_rule=node_search(bridge_id,o_cls_rule);
...
uint32_t ecall_rule_calculate_tag(int bridge_id, struct cls_rule *o_cls_rule,const struct flow *flow,int table_id)
{
...
void ecall_miniflow_expand(int bridge_id, struct cls_rule *o_cls_rule,struct flow *flow){
...
void ecall_SGX_table_dpif(int bridge_id, int n_tables){
...
int ecall_table_update_taggable(int bridge_id, uint8_t table_id){
	...
	if (SGX_table_dpif[bridge_id][table_id].catchall_table != catchall || SGX_table_dpif[bridge_id][table_id].other_table != other) {
...
int ecall_is_sgx_other_table(int bridge_id, int id){
...
uint32_t ecall_rule_calculate_tag_s(int bridge_id, int id, const struct flow *flow)
{
	...
	if(minimask_is_catchall(&SGX_table_dpif[bridge_id][id].other_table->mask)){
		...
		uint32_t hash= flow_hash_in_minimask(flow, &SGX_table_dpif[bridge_id][id].other_table->mask, SGX_table_dpif[bridge_id][id].basis);
		return hash;
	}
}
...
//DEBUG
void ecall_table_dpif_init(int bridge_id){
...
uint16_t ecall_minimask_get_vid_mask(int bridge_id, struct cls_rule *o_cls_rule){
	//Retrieve the cls_rule
	struct sgx_cls_rule *sgx_cls_rule;
	sgx_cls_rule=node_search(bridge_id, o_cls_rule);
...
uint16_t ecall_miniflow_get_vid(int bridge_id, struct cls_rule *o_cls_rule){
	struct sgx_cls_rule *sgx_cls_rule;
	sgx_cls_rule=node_search(bridge_id, o_cls_rule);
...
int ecall_ofproto_get_vlan_c(int bridge_id){
...
void ecall_ofproto_get_vlan_r(int bridge_id, uint16_t *buf,int elem){
	struct oftable *oftable;
	int i;
	int p=0;
	for(i=0;i<SGX_n_tables[bridge_id];i++) {
