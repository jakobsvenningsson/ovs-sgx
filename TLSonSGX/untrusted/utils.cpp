 ...
 */
int
initialize_enclave(sgx_enclave_id_t * eid){
    ...
    HANDLE token_handler =
      CreateFileA(token_path, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS, NULL,
      NULL);
    ...
    ret = sgx_create_enclave("tls_enclave.signed.so", SGX_DEBUG_FLAG, &token, &updated, eid, NULL);
    if (ret != SGX_SUCCESS) {
